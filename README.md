...
本项目采用 **ReAct** (Reasoning + Acting) 模式，让 Agent 能够进行显式推理并通过工具执行完成复杂任务。

### 核心概念

ReAct 由 **Princeton + Google** 2022 年提出，核心思想是让 LLM 在执行任务时进行**显式推理**，而不是直接生成工具调用。
...
| 阶段 | 说明 | 代码体现 |
|------|------|---------|
| Thought | LLM 输出 thinking 文本 | `🧠 Thinking:` 显示 |
| Action | 调用工具 | `🔧 Tool Call:` |
| Observe | 获取结果 | `✓ Result:` |
| Reason | 推理下一步 | 进入下一轮 LLM 调用 |
...
|------|------|
| **ReAct** | 推理 + 行动 + 观察 |
| **Reflexion** | ReAct + 自我反思 + 记忆 |
| **CoT** | Chain of Thought - 仅推理不行动 |
| **ToT** | Tree of Thought - 多路径探索 |
| **Plan-and-Execute** | 规划阶段 + 执行阶段分离 |
